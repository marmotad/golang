...
//遍历原生字符串，统计每个字符出现的次数
func main() {
	str := `I am happy to join with you today in what will go down in history as the greatest demonstration for freedom in the history of our nation.
...
`
	chars := map[rune]int{}
	for _, ok := range str {
		if (ok >= 'A' && ok <= 'Z') || (ok >= 'a' && ok <= 'z') {
			chars[ok] += 1
		}
	}
	fmt.Println(chars)

	for ch, cnt := range chars {
		fmt.Printf("%c:%d\n", ch, cnt)
	}
	fmt.Println("--------------------------")
	chars01 := map[int]rune{}
	for ch, cnt := range chars {
		chars01[cnt] += ch
	}
	for ch, cnt := range chars01 {
		fmt.Printf("%c:%d\n", ch, cnt)
	}
	//fmt.Print(chars01)
